...
class TestFact(unittest.TestCase):
    ...
    def test_is_factorial(self):
        actual = timer.runTimedResult(fact.is_factorial, 87178291200)
        expected = True
        self.assertEqual(actual, expected, "is_factorial")

    def test_get_factors(self):
        actual = timer.runTimedResult(fact.get_factors, 121)
        expected = [1, 11]
        self.assertEqual(actual, expected, "get_factors")


    def test_get_factorial(self):
        actual = timer.runTimedResult(fact.get_factorial, 10)
        expected = 362880
