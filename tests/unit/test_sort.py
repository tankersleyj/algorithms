...
class TestSort(unittest.TestCase):
    ...
    def test_sort_bubble(self):
        actual = timer.run_timed_result(sort.sort_bubble, unordered_list)
        expected = ordered_list
        self.assertEqual(actual, expected, 'sort_bubble')

    def test_sort_instant(self):
        actual = timer.run_timed_result(sort.sort_instant, unordered_list)
        expected = ordered_list
        self.assertEqual(actual, expected, 'sort_instant')

    def test_sort_merge(self):
        actual = timer.run_timed_result(sort.sort_merge, unordered_list)
        expected = ordered_list
        self.assertEqual(actual, expected, 'sort_merge')

    def test_sort_quick(self):
        actual = timer.run_timed_result(sort.sort_quick, unordered_list)
        expected = ordered_list
