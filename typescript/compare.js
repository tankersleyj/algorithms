...
function compareArrays(a1, a2) {
    var result = 0, maxIndex = Math.max(a1.length, a2.length);
...
function compareValuesAsNumber(value1, value2) {
    var number1 = (value1) ? Number(value1) : 0;
    var number2 = (value2) ? Number(value2) : 0;
...
function compareArraysAs(a1, a2, method) {
    var result = 0, compareFunc, maxIndex = Math.max(a1.length, a2.length);
    if (typeof method === "function") {
        compareFunc = method;
    } // user defined
    else if (method === "Number") {
        compareFunc = compareValuesAsNumber;
    } // cast Number
    else if (method === "String") {
        compareFunc = compareValuesAsString;
    } // cast String
    else {
        compareFunc = compareValues;
    }
    ; // no explicit cast
    for (var i = 0; i < maxIndex; i++) {
