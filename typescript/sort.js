...
function quickSort(items) {
    function _partition(items, left, right) {
        var midVal = items[Math.floor((left + right) / 2)], i = left, j = right;
        while (i < j) {
            while (items[i] < midVal) {
                i++;
            }
            while (items[j] > midVal) {
                j--;
            }
            if (i < j) {
                swapItemValues(items, i++, j--);
    ...
    function _recur(items, left, right, priorState) {
        if (priorState === void 0) { priorState = undefined; }
        var middle, newState;
        if (items.length > 1) {
            ...
            if (JSON.stringify(newState) != JSON.stringify(priorState)) {
                if (left < middle) {
                    _recur(items, left, middle, newState);
                }
                if (right > middle + 1) {
                    _recur(items, middle + 1, right, newState);
                }
            }
        }
        return items;
...
(function testSort() {
    var orderedItems = [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 15, 20, 25, 30, 31, 32];
    var unOrderedItems = [10, 20, 31, 4, 6, 5, 15, 2, 12, 25, 3, 30, 32, 1, 7, 8];
    var duplicateItems = [8, 6, 4, 2, 4, 6, 8, 3, 5, 5, 5, 7, 9, 2, 4, 8];
    var negativeItems = [5, 3, 7, -2, 6, 2, 9, 1, -4, -9, 0, 8, -3];
    // bubble sort
